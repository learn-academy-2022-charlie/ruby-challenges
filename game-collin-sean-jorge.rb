...
if user_name == 'trek'
    p 'Into the jungle you encounter the three headed jaguar known as a Triguar. Do you decide to run or fight? Enter: run or fight'
        user_name = gets.chomp
            if user_name == 'run'
                p 'You make the cowardly choice. Now you know that you never turn your back on the powerful Triguar. Your journey is OVA'  # Need to end code here. As of now the ending is displayed after this choice even though the journey is over.
            else 
...
p 'As you proceed on your journey you reach a river crossing. Do you decide to cross the bridge or find an alternate route. Enter: bridge or alternate'
    user_name = gets.chomp
        if user_name == 'bridge'
            p 'The bridge is military grade and breaks in half. You are now in the river and get eaten by dinosaur sized piranahs. Your journey if OVA'
        elsif
